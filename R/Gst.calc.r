Gst.calc <- function(input,sample.sizes){
...
#           Hs.actual,Ht.actual,sample.size -> Gst();
#           allelefrequency -> Ht();
...
#------------------------------------------------------------------------------------------------------------------------------  
          ...
          Gst.values<-numeric(0)
          
                    # This vector will be filled with the per locus calculated Gst
                    # values.
          ...
          for (l in 1:max(loci.numbers)){
                                          ...
                                          Gst.one.locus<-Gst(Hs.actual,Ht.actual,sample.size)
                                          
                                          Gst.values<-rbind(Gst.values,cbind(Gst.one.locus,names(sample.sizes2)[l]))
                                          
                                                    # The Gst values for each locus are combined and the locus names
                                                    # are added to the Gst value they belong to.
                                          }
                                          
          Gst.values<-as.data.frame(Gst.values)
          colnames(Gst.values)=c("Gst","locus")
          
          Gst.over.loci<-mean(as.numeric(as.vector((Gst.values$Gst))),na.rm=TRUE)
          ...
          G.values<-list(Gst.values,Gst.over.loci)
          names(G.values)=c("Gst.values.for.loci","Mean.Gst.value")
          invisible(G.values)
