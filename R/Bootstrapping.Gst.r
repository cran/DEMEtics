Bootstrapping.Gst <- function (tab, bt=1000){
...
#           Gst.means, Gst.locis, significance.levels -> Workspace;
#------------------------------------------------------------------------------------------------------------------------------  
...
Empirical.values <- Gst.calc(allelefrequency,sample.sizes)

          # Dest.Chao values are calculated for each locus as well as
          # averaged over all loci from the empirical data table.

Gst.locus.empirical <- G.values[[1]]
Gst.means.empirical <- G.values[[2]]  

Gst.locus<-vector("list",length=bt)

          # This vector will be filled with the per locus calculated Gst values. 
          
Gst.means<-vector(length=bt)

          # This vector will be filled with the Gst values calculated over all
          # loci.        
...
for (repetition in 1:bt){
                            ...
                            Gst.locus[[repetition]]<-G.values[[1]]
                            Gst.means[repetition]<-G.values[[2]]

                            }

assign("Gst.means",Gst.means,pos = ".GlobalEnv")

          # The list "Gst.locus" and the vector "Gst.means" are 
          # both assigned to the workspace of R in order to be
          # available for further calculations.

critical.values.means <-
cbind(Gst.means.empirical-1.96*(sd(as.numeric(as.vector(Gst.means)),na.rm=TRUE)),Gst.means.empirical+1.96*(sd(as.numeric(as.vector(Gst.means)),na.rm=TRUE)))
...
Gst.loci <- numeric(0)
for (n in 1:bt){
                  Gst.loci<-rbind(Gst.loci,Gst.locus[[n]])
                  }

          # The Gst values for the several loci are combined in a single
          # data frame.
          
assign("Gst.loci",Gst.loci,pos = ".GlobalEnv") 

          # The data.frame "Gst.loci" is saved with its according name
...
for (l in 1: number.loci){
          ...
          # The critical  value for every locus is calculated separately
                          ...
                          rbind(critical.value.loci,
                                cbind(as.numeric(as.vector(Gst.locus.empirical$Gst[l]))-1.96*(sd(as.numeric(as.vector(Gst.loci2[[l]]$Gst)),na.rm=TRUE)),
                                      as.numeric(as.vector(Gst.locus.empirical$Gst[l]))+1.96*(sd(as.numeric(as.vector(Gst.loci2[[l]]$Gst)),na.rm=TRUE))))
