pair.pops.Gst.est <- function (filename,object=FALSE,format.table=TRUE,p.Val=TRUE,bt=1000){
...
#              tab -> allelefreq();
#              y.pair -> Bootstrapping.Gst.est();
#              allelefrequency.pair, sample.sizes.pair -> Gst.est.calc();
...
#              pairwise.Gst.est.loci.[date].txt -> working directory (file);
#              pairwise.Gst.est.mean.[date].txt -> working directory (file);
...
#------------------------------------------------------------------------------------------------------------------------------  
          ...
          # Function that calculates the Gst.est value for each locus separately and
...
for (i in 1:pairwise.comparisons){
          ...
          # seperately.
                                    ...
                                    Bootstrapping.Gst.est(y.pair, bt)
                                    
                                              # Confidence levels for the Gst.est values for the actual pair of
                                              # populations is returned in the object 'confidence.limits'.
                                              # The bootstrap values are available from the objects 'Gst.est.loci'
                                              # and 'Gst.est.means'.
                                              
                                    Gst.est.calc(allelefrequency.pair,sample.sizes.pair)
                                    
                                              # The Gst.est values for every locus and the mean Gst.est value over all
                                              # loci is calculated.
                                    ...
                                    for (l in 1:number.loci){
                                                              ...
                                                              Gv.loci=rbind(Gv.loci,cbind(as.numeric(as.vector(G.values[[1]]$Gst.est[l])),
                                                              ...
                                                              names(allelefrequency.pair3)[2],sep=""),confidence.limits[[1]][l,c(1:2)]))
                                                                        ...
                                                                        # In this vector,the Gst.est value for
                                    ...
                                    Result.actual.comparison.locis <- cbind(Gv.loci,as.numeric(as.vector(p.values.loci[,1])))
                                      ...
                                      colnames(Result.actual.comparison.locis) <- c("Gst.est.for.locus","locus","population1","population2",
                                    ...
                                    Result.actual.comparison.mean <- cbind(G.values[[2]],names(allelefrequency.pair.pops)[1],names(allelefrequency.pair.pops)[2],
                                      ...
                                      colnames(Result.actual.comparison.mean) <- c("Mean.Gst.est","population1","population2",
                                           "populationpair","0.95.conf.int.lower","0.95.confint.upper","p.values")  
                                           
                                           
                                    actual.date <- as.Date(Sys.time())
                                    filename.for.loci <- paste("pairwise.Gst.est.loci.",actual.date,sep="")
                                    filename.for.loci <- paste(filename.for.loci,".txt",sep="")
                                    filename.for.loci <- paste(filename,".",filename.for.loci,sep="")
                                    
                                    filename.for.mean <- paste("pairwise.Gst.est.mean.",actual.date,sep="")
                                    ...
                                    names(interm.result) <- c("Gst.est.loci.pairwise.comparison","Gst.est.mean.pairwise.comparison")
                                    ...
                                    cat("\n","----------------------------------------------------------------------------------------------------","\n")  
                                    cat("\n","Gst.est values for the comparison between",names(allelefrequency.pair.pops)[[1]],
                                        "and",names(allelefrequency.pair.pops)[[2]],"\n","for every locus saved in",
                                        "'",filename.for.loci,"'","\n",sep=" ")
                                    cat("\n","Mean Gst.est values for the comparison between",names(allelefrequency.pair.pops)[[1]],
                                    ...
                                    Gv.mean=rbind(Gv.mean,cbind(G.values[[2]],names(allelefrequency.pair.pops)[1],names(allelefrequency.pair.pops)[2],
                                              ...
                                              # In this vector, the Gst.est value over all loci, population one and
...
colnames(Gv.locis)=c("Gst.est.for.locus","locus","population1","population2",
"populationpair","0.95.conf.int.lower","0.95.conf.int.upper","p.values")            

Gv.mean=as.data.frame(Gv.mean)
colnames(Gv.mean)=c("Mean.Gst.est","population1","population2",
"populationpair","0.95.conf.int.lower","0.95.conf.int.upper","p.values")
       
Gv.pairwise=list(Gv.locis,Gv.mean)
names(Gv.pairwise)=c("Gst.est.loci.pairwise.comparison","Gst.est.mean.pairwise.comparison")
...
Gst.est.pairwise.adjusted <- Dv.pairwise.adjusted



names(Gst.est.pairwise.adjusted)=c("Gst.est.loci.pairwise.comparison","Gst.est.mean.pairwise.comparison")

          
colnames(Gst.est.pairwise.adjusted[[1]]) <- c("Gst.est.for.locus","locus","population1","population2",
       "populationpair","0.95.conf.int.lower",
       "0.95.conf.int.upper","p.values","p.bonferroni","p.holm","p.hommel","pBH")  
       
colnames(Gst.est.pairwise.adjusted[[2]]) <- c("Mean.Gst.est","population1","population2",
       "populationpair","0.95.conf.int.lower","0.95.conf.int.upper","p.values","p.bonferroni","p.holm","p.hommel","pBH")   
       
rownames(Gst.est.pairwise.adjusted[[1]]) <- seq(1,length(Gst.est.pairwise.adjusted[[1]][,1]))            
rownames(Gst.est.pairwise.adjusted[[2]]) <- seq(1,length(Gst.est.pairwise.adjusted[[2]][,1]))  
cat("\n","----------------------------------------------------------------------------------------------------","\n")         

cat("\n","\n","+++++++++++++++++++++++++++++++++++++++++++  END RESULT  +++++++++++++++++++++++++++++++++++++++++++","\n","\n")  
       
print(Gst.est.pairwise.adjusted)
     
assign("Gst.est.pairwise.adjusted",Gst.est.pairwise.adjusted,pos = ".GlobalEnv")

          # The list 'Gst.est.pairwise.adjusted' is assigned to the workspace
...
write.table(as.data.frame(as.matrix(Gst.est.pairwise.adjusted[[1]])),file=filename.for.loci, append = TRUE, quote = FALSE, sep = " ", na = "NA", dec = ".", row.names = FALSE, col.names = TRUE)
write.table(as.data.frame(as.matrix(Gst.est.pairwise.adjusted[[2]])),file=filename.for.mean, append = TRUE, quote = FALSE, sep = " ", na = "NA", dec = ".", row.names = FALSE, col.names = TRUE)
cat("\n","----------------------------------------------------------------------------------------------------","\n","\n")         
          ...
          # seperately.
                                    ...
                                    for (l in 1:number.loci){
                                     ...
                                     Result.actual.comparison.locis <- Gv.loci
                                      Result.actual.comparison.locis <- as.data.frame(Result.actual.comparison.locis)
                                      colnames(Result.actual.comparison.locis) <- c("Gst.est.for.locus","locus","population1","population2","populationpair")            
...
Gst.est.pairwise.adjusted <- Gv.pairwise
