pair.pops.Dest <- function (filename,object=FALSE,format.table=TRUE,p.Val=TRUE,bt=1000){
...
#              allelefrequency.pair, sample.sizes.pair -> Dest.calc();
...
#              pairwise.Dest.loci.[date].txt -> working directory (file);
#              pairwise.Dest.mean.[date].txt -> working directory (file);
...
#------------------------------------------------------------------------------------------------------------------------------  
          ...
          # Function that calculates the Dest value for each locus separately and
          # the mean Dest value over all loci, for each possible pairwise population 
          # comparison.
...
for (i in 1:pairwise.comparisons){
          ...
          # seperately.
                                  ...
                                  Bootstrapping.Dest(y.pair, bt)
                                  
                                            # Confidence levels for the Dest values for the actual pair of
                                            # populations is returned in the object 'confidence.limits'.
                                            # The bootstrap values are available from the objects 'Dest.loci'
                                            # and 'Dest.means'.
                                            
                                  Dest.calc(allelefrequency.pair,sample.sizes.pair)
                                  
                                            # The Dest values for every locus and the mean Dest value over all
                                            # loci is calculated.
                                  ...
                                  for (l in 1:number.loci){
                                                            ...
                                                            Dv.loci=rbind(Dv.loci,cbind(as.numeric(as.vector(D.values[[1]]$Dest[l])),
                                                            ...
                                                            names(allelefrequency.pair3)[2],sep=""),confidence.limits[[1]][l,c(1:2)]))
                                                                      ...
                                                                      # In this vector,the Dest value for
                                  ...
                                  Result.actual.comparison.locis <- cbind(Dv.loci,as.numeric(as.vector(p.values.loci[,1])))
                                    ...
                                    colnames(Result.actual.comparison.locis) <- c("Dest.for.locus","locus","population1","population2",
                                  ...
                                  Result.actual.comparison.mean <- cbind(D.values[[2]],names(allelefrequency.pair.pops)[1],names(allelefrequency.pair.pops)[2],
                                    ...
                                    colnames(Result.actual.comparison.mean) <- c("Mean.Dest","population1","population2",
                                         "populationpair","0.95.conf.int.lower","0.95.conf.int.upper","p.values")  
                                         
                                         
                                  actual.date <- as.Date(Sys.time())
                                  filename.for.loci <- paste("pairwise.Dest.loci.",actual.date,sep="")
                                  filename.for.loci <- paste(filename.for.loci,".txt",sep="")
                                  filename.for.loci <- paste(filename,".",filename.for.loci,sep="")
                                  
                                  filename.for.mean <- paste("pairwise.Dest.mean.",actual.date,sep="")
                                  ...
                                  names(interm.result) <- c("Dest.loci.pairwise.comparison","Dest.mean.pairwise.comparison")
                                  ...
                                  cat("\n","Dest values for the comparison between",names(allelefrequency.pair.pops)[[1]],
                                      "and",names(allelefrequency.pair.pops)[[2]],"\n","for every locus saved in",
                                      "'",filename.for.loci,"'","\n",sep=" ")
                                  cat("\n","Mean Dest values for the comparison between",names(allelefrequency.pair.pops)[[1]],
                                  ...
                                  Dv.mean=rbind(Dv.mean,cbind(D.values[[2]],names(allelefrequency.pair.pops)[1],names(allelefrequency.pair.pops)[2],
                                            ...
                                            # In this vector, the Dest value over all loci, population one and
...
colnames(Dv.locis)=c("Dest.for.locus","locus","population1","population2",
"populationpair","0.95.conf.int.lower","0.95.conf.int.upper","p.values")            
       
Dv.mean=as.data.frame(Dv.mean)
colnames(Dv.mean)=c("Mean.Dest","population1","population2",
"populationpair","0.95.conf.int.lower","0.95.conf.int.upper","p.values")
       
Dv.pairwise=list(Dv.locis,Dv.mean)
names(Dv.pairwise)=c("Dest.loci.pairwise.comparison","Dest.mean.pairwise.comparison")
...
names(D.pairwise.adjusted)=c("Dest.loci.pairwise.comparison","Dest.mean.pairwise.comparison")

          
colnames(D.pairwise.adjusted[[1]]) <- c("Dest.for.locus","locus","population1","population2",
       "populationpair","0.95.conf.int.lower","0.95.conf.int.upper","p.values","p.bonferroni","p.holm","p.hommel","pBH")  
       
colnames(D.pairwise.adjusted[[2]]) <- c("Mean.Dest","population1","population2",
       "populationpair","0.95.conf.int.lower","0.95.conf.int.upper","p.values","p.bonferroni","p.holm","p.hommel","pBH")   
...
for (i in 1:pairwise.comparisons){
          ...
          # seperately.
                                  ...
                                  Result.actual.comparison.locis <- Dv.loci
                                    Result.actual.comparison.locis <- as.data.frame(Result.actual.comparison.locis)
                                    colnames(Result.actual.comparison.locis) <- c("Dest.for.locus","locus","population1","population2","populationpair")            
