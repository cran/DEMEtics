all.pops.Gst <- function (filename,object=FALSE,format.table=TRUE, p.Val=TRUE, bt=1000){
...
#              tab -> Bootstrapping.Gst(), allelefreq();
#              allelefrequency, sample.sizes -> Gst.calc();
#              empirical.value.loci,bootstrapped.values.loci,empirical.value,bootstrapped.values -> p.value();


# Output:
#              Gst.all.pops -> Workspace;
#              bootstrap.time.output -> screen (print);
#              names(tab.pops) -> screen (print);
#              Gst.all.pops -> screen (print);
#              all.pops.Gst.loci.[date].txt -> working directory (file);
#              all.pops.Gst.mean.[date].txt -> working directory (file);
#              Gst.sample.sizes.[date].txt -> working directory (file);
#              Gst.allelefrequencies.[date].txt -> working directory (file);
#------------------------------------------------------------------------------------------------------------------------------  
...
if (format.table==TRUE){
          ...
          # This function will calculate the Gst values for every locus and
          # the mean Gst value over all loci for all the populations that 
          # have been sampled.
...
Bootstrapping.Gst(tab,bt)

          # The confidence limits of the Gst values for the several loci and 
          ...
          # the Gst value from those resampled data tables.
...
Gst.calc(allelefrequency,sample.sizes)
          ...
          # The Gst values for the several loci and the mean Gst value of 
...
for (l in 1:number.loci){
          ...
          # loci.
                          ...
                          bootstrapped.values.loci=as.numeric(as.vector(Gst.loci2[[l]]$Gst))
                                    ...
                                    # The bootstrapping values (Gst values) that have been obtained for 
                                    # the several loci are assigned to a vector.
          
                          empirical.value.loci=as.numeric(as.vector(G.values[[1]]$Gst[l]))

                                    # The empirical Gst values for the several loci are assigned to a
                                    # vector.
                          ...
                          if (is.nan(empirical.value.loci)){
                                    ...
                                    # This function returns the p-value for the actual empirical Gst value
...
bootstrapped.values=Gst.means
          ...
          # The mean Gst values over all loci that have been obtained from
          # the bootstrapping.
          
empirical.value=G.values[[2]]

          # The empirical mean Gst value over all loci. 
...
colnames(D.loci)=c("Gst","locus","0.95.conf.int.lower","0.95.conf.int.upper","p.value")

          # A table is created that contains the Gst values for the several
...
colnames(D.mean)=c("Gst.mean","0.95.conf.int.lower","0.95.conf.int.upper","p.value")

          # A table is created that contains the mean Gst value over all
          # loci with the according confidence levels and the p-value obtained
          # from the bootstrapping analysis.

Gst.all.pops=list(D.loci,D.mean)
names(Gst.all.pops)=c("Gst.loci.over.all.populations","Gst.mean.over.all.populations")

          # These two tables are combined in a single list.

assign("Gst.all.pops",Gst.all.pops,pos = ".GlobalEnv") 

          # The object 'Gst.all.pops', a list, is assigned to the R workspace.
...
filename.for.allelefrequencies <- paste("Gst.allelefrequencies.",actual.date,sep="")
...
write.table(as.data.frame(as.matrix(Gst.all.pops[[1]])),file=filename.for.loci, append = TRUE, quote = FALSE, sep = " ", na = "NA", dec = ".", row.names = FALSE, col.names = TRUE)
write.table(as.data.frame(as.matrix(Gst.all.pops[[2]])),file=filename.for.mean, append = TRUE, quote = FALSE, sep = " ", na = "NA", dec = ".", row.names = FALSE, col.names = TRUE)
